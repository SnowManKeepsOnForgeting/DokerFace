...
async def test_player_can_update_avatar_text_and_background_color() -> None:
    session = AsyncMock(spec=AsyncSession)
    current_account = make_account(1, "alice")
    app = build_app(session, current_account)
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.patch(
            "/api/v1/me/profile",
            json={"avatar_text": "A\n🂡", "avatar_background_color": "#12abEF"},
        )

    assert response.status_code == 200
    assert response.json()["avatar_text"] == "A\n🂡"
    assert response.json()["avatar_background_color"] == "#12ABEF"
    assert current_account.profile is not None
    assert current_account.profile.avatar_text == "A\n🂡"
    assert current_account.profile.avatar_background_color == "#12ABEF"
    session.commit.assert_awaited_once()
...
@pytest.mark.parametrize(
    ...
    [
        {"avatar_text": " \n  "},
        {"avatar_background_color": "#12345"},
        {"avatar_background_color": "red"},
    ],
)
async def test_avatar_update_rejects_invalid_values(payload: dict[str, str]) -> None:
    session = AsyncMock(spec=AsyncSession)
