...
补充筹码和历史保留周期仍需单独定稿，运行时不得自行推断这些行为；牌局中的 `Quit` 行为已定稿并由服务端实时命令执行。
...
| 用途 | 选型 | 原因 |
| --- | --- | --- |
| 框架 | [React](https://github.com/facebook/react) 19.2 + TypeScript 6 | 组件生态成熟，适合实时牌桌和后台页面；TypeScript 6 与首版质量和生成工具链处于共同支持范围 |
| 编译优化 | React Compiler 1 | 官方稳定编译期自动记忆化，减少手工 `memo`、`useMemo` 和 `useCallback` |
| 构建工具 | [Vite](https://github.com/vitejs/vite) 8 + Rolldown | Rust 构建内核、开发反馈快，直接产出由 Caddy 托管的 SPA 静态资源 |
| 路由 | [React Router](https://github.com/remix-run/react-router) 8 Data Mode | 提供类型化懒加载、鉴权和错误边界，不引入第二套服务端框架 |
| REST 数据 | [TanStack Query](https://github.com/TanStack/query) 5 | 缓存资料、历史、排行榜和后台查询 |
| 实时状态 | [Zustand](https://github.com/pmndrs/zustand) 5 | 保存当前房间连接和服务端牌局快照 |
| 实时客户端 | socket.io-client | 与 python-socketio 协议一致，提供自动重连 |
| 表单 | React Hook Form 7 + Zod 4 | 房间规则、登录和后台表单校验 |
| UI 基础 | [Radix UI](https://github.com/radix-ui/primitives) + 精选 shadcn/ui 源码 + [Tailwind CSS](https://github.com/tailwindlabs/tailwindcss) 4 | 使用可访问、项目自有的交互组件，同时保持牌桌界面可定制 |
...
| REST 契约生成 | `@hey-api/openapi-ts` | 从 FastAPI OpenAPI 生成 TypeScript 6 类型和 Fetch SDK；Query hooks 保持项目自有 |
...
| 前端单元测试 | Vitest 4 + React Testing Library + MSW |
| 浏览器端到端测试 | Playwright 1.61+ |
| Python 代码检查 | Ruff + Pyright |
| TypeScript 代码检查 | ESLint 10 + typescript-eslint + Prettier 3 + TypeScript 6 strict mode |

所有依赖必须固定版本并提交锁文件。PokerKit 只能通过项目适配器引用，避免其 API 变化扩散到房间和网络层。引入依赖前检查许可证、维护状态、发布记录和测试情况；仓库公开但缺少许可证的代码不得复制进项目。

首版固定 TypeScript 6，不使用 TypeScript 7，也不忽略 peer dependency 警告。只有
`typescript-eslint`、REST 契约生成器、Vite、Vitest 和编辑器工具均正式声明支持后，才以独立提交
升级 TypeScript 主版本。React Compiler 使用官方 Vite 集成和 `eslint-plugin-react-hooks` 推荐规则；
默认依赖编译器优化，只有性能测量证明需要时才手写记忆化。
...
- 赢家通杀与固定手数两种整场比赛模式。
- 盲注按手牌数量翻倍。
- 决策计时、超时默认动作（可免费过牌时过牌，否则弃牌）和不限时房间的断线 60 秒兜底。
