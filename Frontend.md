...
9. 默认依赖 React Compiler 优化组件，不让 LLM 或开发者无依据地堆叠手工记忆化。

## 3. 技术栈

以下主版本基线于 2026-07-16 根据官方发布状态和上游 peer dependency 重新核对。脚手架阶段锁定
精确解析结果并提交 `pnpm-lock.yaml`；后续升级必须独立提交并通过类型、组件、构建和端到端测试。

| 范围 | 选型 | 用途与约束 |
| --- | --- | --- |
| 运行时 | Node.js 24 LTS | 本地开发和 CI 构建，不进入浏览器运行时 |
| 包管理 | pnpm 10 + Corepack | 严格锁文件，CI 使用 `--frozen-lockfile` |
| 框架 | React 19.2 + TypeScript 6.0.x | 仅使用函数组件，开启 TypeScript strict |
| 编译优化 | React Compiler 1.x | 使用官方 Vite 集成自动记忆化，配套 hooks lint 规则 |
| 构建 | Vite 8 + Rolldown | Rust 构建内核、本地代理和生产静态资源构建 |
| React 插件 | `@vitejs/plugin-react` 6 | 使用官方 Rolldown/Babel Compiler 集成，不采用非官方转换插件 |
| 路由 | React Router 8 Data Mode | 路由懒加载、鉴权和错误边界，不使用 Framework/SSR Mode |
| REST 状态 | TanStack Query 5 | 缓存、失效、分页、请求生命周期 |
| 实时状态 | Zustand 5 | 单一 Socket.IO 会话、房间和牌局权威快照 |
| 实时传输 | socket.io-client | Cookie 认证、确认响应、断线和自动重连 |
| 表单 | React Hook Form 7 + Zod 4 | 登录、资料、建房和管理表单 |
| REST 契约 | `@hey-api/openapi-ts` | 生成 TypeScript 6 类型和 Fetch SDK；Query hooks 由项目维护 |
| 实时契约 | Pydantic JSON Schema + 生成的 Zod schema | 校验所有入站事件和 `schema_version` |
| 样式 | Tailwind CSS 4 | CSS-first 设计 token、响应式布局和状态样式 |
| 交互基础 | Radix UI + 精选 shadcn/ui 源码 | 项目持有的 Dialog、Dropdown、Tabs、Tooltip、Toast 等可访问组件 |
| 图标 | Lucide React | 操作按钮使用统一图标和工具提示 |
| 动画 | Motion | 发牌、筹码、结算和表情；只消费状态差异 |
| 表格 | TanStack Table | 排行榜、历史和管理端数据表 |
| 单元/组件测试 | Vitest 4 + React Testing Library + MSW | 状态归并、表单、路由和 REST 场景 |
| 端到端测试 | Playwright 1.61+ | 在真实 API/Socket.IO 上验证关键旅程 |
| 代码质量 | ESLint 10 + typescript-eslint + Prettier 3 + `tsc --noEmit` | 保留 React Compiler 官方 lint 规则，CI 中零警告 |

不采用 Next.js、TanStack Start、React Router Framework Mode、Redux、GraphQL、微前端、
Service Worker 离线牌局或浏览器扑克引擎。这些能力不解决当前规模的问题，反而会扩大状态所有权
和部署边界。Biome 和 Oxlint 保持候选工具，不在首版替换 ESLint/Prettier，以免失去 React Compiler
官方 hooks lint 覆盖。

### 3.1 TypeScript 版本策略

首版直接固定 TypeScript `6.0.x`，不安装 TypeScript 7，也不通过 pnpm 配置忽略 peer dependency。
当前 typescript-eslint 正式支持 `<6.1.0`，`@hey-api/openapi-ts` 明确支持 TypeScript 6，因此这组
组合可以在严格安装下工作。TypeScript 7 虽然已经稳定发布，但只有在 typescript-eslint、契约生成、
Vite、Vitest 和编辑器插件全部正式声明支持后，才通过独立提交升级。

依赖版本以锁文件为准，表格中的主版本不是自动升级范围。React Router、Tailwind 和 Vite 跨主版本
写法差异较大，给 LLM 的任务必须引用仓库内现有示例，禁止生成旧版 Router 6、Tailwind 3 或
Create React App 配置。
...
开发环境的重复 effect 建立两条连接。React Compiler 默认负责组件和 hooks 的记忆化；手写
`memo`、`useMemo` 或 `useCallback` 必须有稳定身份需求或性能测量依据。
...
│   │   ├── errors.ts
│   │   ├── query-keys.ts
│   │   └── generated/           # Hey API 生成的类型和 Fetch SDK
│   ├── components/
│   │   ├── ui/
│   │   ├── AppShell.tsx
│   │   ├── Avatar.tsx
...
- 后端 OpenAPI 是 REST 类型和 SDK 的唯一来源，`@hey-api/openapi-ts` 生成目录禁止手工编辑。
- 项目自有的 client 配置统一设置同源基础地址和 `credentials: "include"`，生成的 Fetch SDK 只经该
  配置访问网络。
- 错误适配器把 FastAPI `detail`、网络错误和未知响应转换为稳定的 `ApiError`。
- 开发与 CI 执行契约生成后检查 `git diff --exit-code`，防止后端 schema 已变而生成物未提交。
- 不生成 TanStack Query hooks；query key、失效范围和 mutation 重试策略由对应 feature 显式维护。
...
4. 公共快照可以推进版本；同一手牌保留最近的本人底牌，但只有与公共快照同版本的私有快照
   可以提供 `legal_actions`。
5. 同版本公共快照不能覆盖已经收到的私有字段；手牌 ID 变化时必须立即清除上一手底牌。
...
- 等待室仅在至少 1280 CSS px 的宽屏使用玩家区与聊天区双栏；更窄窗口按房间摘要、
  玩家列表、聊天纵向排列。聊天面板高度有界，消息只在面板内部滚动，不能推动页面持续增长。
...
- 使用官方 Vite 8 React Compiler 模板创建 React 19.2 + TypeScript 6 工程，锁定依赖。
- 配置 TypeScript 6 strict、ESLint 10、typescript-eslint、React Compiler hooks 规则、Prettier 3、
  Vitest 4、Playwright 和路径别名。
- 验证 pnpm 安装无 peer dependency 警告，禁止使用 overrides 掩盖不兼容主版本。
