...
仓库中的 `deploy/Caddyfile.example` 和 `deploy/compose.example.yml` 提供部署默认配置。
使用前需要将它们复制为已被 Git 忽略的本地配置文件。公网生产部署还需要按下文修改域名
和端口。
...
界面提供英文与简体中文两种语言。语言偏好按浏览器保存，不写入数据库。

- 可在侧边栏（桌面端与移动端）、登录页，或个人资料页的「偏好设置」区块切换。
- 首次访问跟随浏览器语言：中文浏览器直接显示简体中文，其他语言回落英文。手动切换后会记录在
  `localStorage` 的 `dokerface.language` 中。
- 只有前端文案被翻译。后端 API 返回的错误信息是英文，按原样展示；Socket.IO 的实时错误码在前端
  翻译为中文。
- 玩家自己填写的数据不翻译，包括昵称、头像文字、聊天内容、快捷短语与段位徽章名称。

翻译资源位于 `frontend/src/i18n/locales/{en,zh}/`，命名空间划分、扑克术语表与测试约定见
`Frontend.md`。
